            ...
            if step > 0 and step % args.valid_step == 0 and rank == 0:
                ...
                if f1 > best_f1:
                    ...
                    paddle.save(model.state_dict(),
                                '{}/best_role{}.pdparams'.format(args.checkpoints, now))
            step += 1

    # save the final model
    if rank == 0:
        paddle.save(model.state_dict(),
                    '{}/final_role{}.pdparams'.format(args.checkpoints,now))
...
def do_predict():
    ...
    print("============start predict==========")
    # init_ckpt = './role_judge_model/EE1.0/trigger/final1658914069.pdparams'
    init_ckpt = '../ckpt/EE1.0/role/best_role.pdparams'
    ...
    if args.tag_path.split('/')[-1] == 'role_tag.dict':
        ...
        if not os.path.isfile('../data/result/trigger/test_predict_trigger.json'):
            ...
            txts1 = read_by_lines('../data/result/trigger/test_predict_trigger.json')
            ...
            for i in range(len(sentences)):
                if len(txt_head[i]) > 0:
                    for txts in txt_head[i]:
                                result.append({'id':sentences[i]['id'] , 'event_type': txts['event_type'],'text': txts['event_type'] +txts['trigger']
                                                                                 +'：'+sentences[i]['text'].strip()})
                # else:
                #     result.append({'id': sentences[i]['id'], 'text': sentences[i]['text']})
        result = val_generate(data_path=args.predict_data, pre_result=result)
        for i in result:
            print(i)
        print(len(result))
        sentences = result
    ...
    now = int(time.time())
    predict_save_path = '{}/test_predict_role.json'.format(args.predict_save_path)
